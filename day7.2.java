...
class Solution {
    ...
    private static boolean isValid(long targetSum, List<Long> operands, long currentSum, int i) {
        ...
        if (isValid(targetSum, operands, currentSum + operands.get(i), i + 1) ||
             isValid(targetSum, operands, currentSum * operands.get(i), i + 1)) {
            return true;
        }

        var concat = Long.parseLong(Long.toString(currentSum) + Long.toString(operands.get(i)));
        return isValid(targetSum, operands, concat, i + 1);
    }
}
